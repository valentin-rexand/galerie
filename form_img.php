...
<?php
	if(isset($_POST['nom']) && (!empty($_POST['nom'])) && (isset($_POST['auteur'])) && (!empty($_POST['auteur'])) && (isset($_POST['description'])) && (!empty($_POST['edescription'])) && (isset($_FILES['fichier']))){
		$query="INSERT INTO galerie_php (nom, auteur, description, `date`, nom_fichier) 
		VALUES (".$db->quote($_POST['nom']).",".$db->quote($_POST['auteur']).",".$db->quote($_POST['description']).",NOW(),".$db->quote($_FILES['fichier']).")";
	...
	/*if(isset($_FILES['fichier'])){
		move_uploaded_file($_FILES['fichier']['tmp_name'], $_FILES['fichier']['image']);
	}*/
